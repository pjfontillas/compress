(function ($) {
    $(document).ready(function () {
        ...
        $('#compress_bookmark_form').on('submit', function (event) {
            event.preventDefault();
            var source = $('#compress_bookmark_form-javascript').val();
            var output = compress(source);
            $('#compress_bookmark_form-bookmarklet_source').val(output);
            $('#compress_bookmark_fork-bookmarklet_link').attr('href', output).show();
        }).on('reset', function (event) {
            $('#compress_bookmark_fork-bookmarklet_link').hide();
        });
        $('#compress_bookmark_fork-bookmarklet_link').hide();
    });
}(jQuery));